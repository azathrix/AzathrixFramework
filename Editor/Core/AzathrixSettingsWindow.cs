...
{
    /// <summary>
    /// Azathrix 配置浏览窗口（类似 Project Settings）
    /// </summary>
    public class AzathrixSettingsWindow : EditorWindow
    {
        ...
        [MenuItem("Azathrix/设置", false, 100)]
        public static void ShowWindow()
        {
            var window = GetWindow<AzathrixSettingsWindow>("设置");
            ...
            foreach (var assembly in assemblies)
            {
                try
                {
                    foreach (var type in assembly.GetTypes())
                    {
                        ...
                        var settingAttr = type.GetCustomAttribute<ShowSettingAttribute>();
